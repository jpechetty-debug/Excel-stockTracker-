...
class AllocationEngine:
    ...
    def __init__(self, max_position_size: Decimal = Decimal('0.10'), cash_floor: Decimal = Decimal('0.05'), portfolio_size: Decimal = Decimal('1000000.0'), min_position_size: Decimal = Decimal('0.0')):
        self.max_position_size = Decimal(str(max_position_size))
        self.cash_floor = Decimal(str(cash_floor))
        self.portfolio_size = Decimal(str(portfolio_size))
    ...
    def calculate_allocations(self, investment_scores: Dict[str, EngineResult]) -> Dict[str, EngineResult]:
        ...
        for t, res in valid_tickers.items():
            ...
            weight = Decimal(str(investment_score)) * Decimal(str(confidence))
            raw_weights[t] = weight
            total_score += weight
            
        investable_capital = Decimal('1') - self.cash_floor
